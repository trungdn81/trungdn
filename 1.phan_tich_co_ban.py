...
def get_latest_market_price(symbol, max_retries=3, delay=2):
    """
    Lấy giá đóng cửa gần nhất từ TCBS (vnstock).
    Có retry + delay để tránh rate limit.
    """
    for attempt in range(max_retries):
        try:
            ...
            stock = Stock(symbol, source="TCBS")
            df_price = stock.quote.history(symbol=symbol, resolution="1D", start=start, end=end)

            if df_price is not None and not df_price.empty:
                last_close = df_price["close"].dropna().iloc[-1]
                return float(last_close) * 1000 if last_close < 1000 else float(last_close)
            ...
            if "rate limit" in msg.lower():
                ...
                print(f"⏳ Gặp giới hạn API, chờ {wait_seconds} giây...")
                time.sleep(wait_seconds + 1)
            else:
                print(f"❌ Lỗi hệ thống {symbol}: {e}")
                break
        except Exception as e:
            print(f"⚠️ Lỗi khi lấy giá {symbol}: {e}")
            time.sleep(delay)
...
with tab1:
    ...
    if st.button("🔍 Thực hiện định giá"):
        try:
            ...
            # ----- LẤY GIÁ THỊ TRƯỜNG từ TCBS -----
            market_price = get_latest_market_price(symbol)
            price_source_note = "TCBS API" if market_price else None
