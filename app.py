...
drive_cache_id = st.sidebar.text_input(
    ...
    value="1AMrekvoERyho9vpudK-U9UkK712FoGbt",  # 👈 thêm dòng này nếu muốn điền sẵn
    help="Ví dụ: File ID từ https://drive.google.com/file/d/1abcXYZ... => ID là phần giữa"
)
    # download_and_extract_cache("1AMrekvoERyho9vpudK-U9UkK712FoGbt")
...
# === PHẦN 3: Điều kiện lọc nâng cao & xử lý chỉ báo ===
    ...
    if os.path.exists(cache_dir):
        ...
        for file in files:
            ...
            try:
                ...
                if price < extra_filters['price_min'] or price > extra_filters['price_max']: continue
                if change_5d < extra_filters['price_change_5d_min'] or change_5d > extra_filters['price_change_5d_max']: continue
                if volume_ratio < extra_filters['volume_ratio_min']: continue
                if extra_filters['ma50_break'] and not ma50: continue
                if extra_filters['ma100_break'] and not ma100: continue
                if extra_filters['macd_positive'] and not macd_pos: continue
                if rsi < extra_filters['rsi_min']: continue
                if volume_avg < min_volume: continue
...
# === PHẦN 4: Hiển thị biểu đồ kỹ thuật cho mã bất kỳ ===
