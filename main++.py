...
def run_backtest(bctc_file, gia_file0, gia_file1):
    ...
    for symbol, df in bctc_data.items():
        try:
            ...
            eps = get_value(latest, ["EPS"])
            pe = get_value(latest, ["P/E"])
            pb = get_value(latest, ["P/B"])
            roe = get_value(latest, ["ROE"])
            bvps = get_value(latest, ["book_value_per_share"])
        ...
        try:
            ...
            for sheet_name, df in all_sheets.items():
                try:
                    ...
                    eps = get_value(latest, ["eps", "EPS"])
                    pe = get_value(latest, ["pe", "P/E"])
                    pb = get_value(latest, ["pb", "P/B"])
                    roe = get_value(latest, ["roe", "ROE"])
                    bvps = get_value(latest, ["book_value_per_share", "BVPS"])
