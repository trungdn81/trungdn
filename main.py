# 📊 Web App Định giá cổ phiếu - Full 3 Tab
...
st.set_page_config(page_title="Định giá cổ phiếu (Full)", layout="wide")
...
def dinh_gia(eps, pe, pb, bvps, roe):
    ...
    return res
# === Giao diện chính và Tab 1 ===
st.title("📊 Ứng dụng định giá cổ phiếu (Web App Full)")

tab1, tab2, tab3 = st.tabs(["📌 Định giá riêng lẻ (API)", "📈 Backtest", "🏆 Lọc Top"])

with tab1:
    st.subheader("📘 Định giá tự động từ mã cổ phiếu")

    symbol = st.text_input("Nhập mã cổ phiếu (VD: FPT, VNM, DPG)", value="FPT")
        ...
        try:
            ...
            if df.empty:
                ...
                try:
                    ...
                    # Thử lấy theo nhiều key khác nhau
                    ...
                    # Fallback: Lấy giá đóng cửa trong 7 ngày gần nhất
                    try:
...
with tab2:
    st.subheader("📈 Backtest định giá từ dữ liệu Excel")

    col1, col2, col3 = st.columns(3)
    with col1:
        bctc_file = st.file_uploader("📂 File BCTC (nhiều sheet)", type=["xlsx"], key="bctc")
    with col2:
        gia0_file = st.file_uploader("📂 File Giá T0", type=["xlsx"], key="gia0")
    with col3:
        gia1_file = st.file_uploader("📂 File Giá T1", type=["xlsx"], key="gia1")
...
with tab3:
    st.subheader("🏆 Lọc Top định giá từ Excel nhiều sheet")

    excel_file = st.file_uploader("📂 File Excel nhiều mã (mỗi sheet là 1 mã)", type=["xlsx"], key="toploc")
